...
// JARVIS brand theme lives in its own file so upstream merges don't conflict.
import { glassifyTheme } from "./jarvis-glassify";
...
/* JARVIS · Glassify (the signature brand theme) is defined in
   `./jarvis-glassify.ts` and imported at the top of this file, so upstream
   Hermes merges never touch it. It is registered in `BUILTIN_THEMES` below. */
